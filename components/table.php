...
function ui_table(array $headers, string $bodyHtml, array $options = []): string
{
    ...
    return '
    <div id="' . e($tableId) . '" class="data-table max-w-full overflow-hidden rounded-[30px] border border-slate-200/80 bg-white/95 shadow-[0_18px_48px_rgba(15,23,42,.06)]"
        ...
        <div class="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/70 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="relative block w-full sm:max-w-xs">
                ...
                <input id="' . e($searchId) . '" name="' . e($tableId . '_search') . '" type="search" class="w-full rounded-xl border border-slate-200 bg-white py-2 pl-10 pr-3 text-sm text-slate-700 outline-none transition focus:border-sky-400 focus:ring-4 focus:ring-sky-100" placeholder="Cari nama..." data-table-search>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/70 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <p class="whitespace-nowrap text-sm text-slate-500" data-table-meta>Menampilkan 0 dari 0 data</p>
