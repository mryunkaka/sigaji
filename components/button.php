...
function ui_button(string $label, array $options = []): string
{
    ...
    return '<button type="' . e($type) . '" class="inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold transition duration-200 ' . $variants[$variant] . '"' . $attrString . '>' . $iconHtml . '<span>' . $label . '</span></button>';
}
